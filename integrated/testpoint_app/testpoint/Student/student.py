...
def enforce_lockdown():
    ...
    if session.get("role") == "student" and "user_id" in session:
        ...
        cursor.execute(
            "SELECT block_id FROM students WHERE student_id = %s", (student_id,)
        )
        res = cursor.fetchone()
        cursor.close()
        connection.close()
...
def setup_account():
    ...
    if request.method == "POST":
        ...
        if stats and stats["current_count"] >= stats["capacity"]:
            flash(
                "Sorry, this block just reached its capacity. Please select another block.",
                "warning",
            )
            return redirect(url_for("student.setup_account"))
        ...
        cursor.execute(
            "UPDATE students SET block_id = %s WHERE student_id = %s",
            (block_id, student_id),
        )
        connection.commit()
        cursor.close()
...
def student_dashboard():
    ...
    try:
        cursor.execute(
            "SELECT COUNT(*) as count FROM enrollments WHERE student_id = %s AND status = 'active'",
            (student_id,),
        )
        ...
        # Protection against division by zero using NULLIF and Base 50 conversion
        cursor.execute(
            """
            SELECT AVG(percentage) as overall_avg FROM (
                SELECT (50 + (ea.score / NULLIF((SELECT COUNT(*) FROM attempt_questions WHERE attempt_id = ea.attempt_id), 0) * 50)) as percentage
        ...
        overall_avg = (
            round(float(avg_res["overall_avg"]), 1)
            if avg_res and avg_res["overall_avg"]
            else 0
        )
        ...
        # Recent results updated with Base 50 conversion in selection [1]
        cursor.execute(
            """
        ...
        # Performance trend using Base 50 conversion
        cursor.execute(
            """
            SELECT e.title,
                   COALESCE((50 + (ea.score / NULLIF((SELECT COUNT(*) FROM attempt_questions WHERE attempt_id = ea.attempt_id), 0) * 50)), 50) as percentage
                FROM exam_attempts ea
                JOIN exams e ON ea.exam_id = e.exam_id
                WHERE ea.student_id = %s AND ea.status = 'finished' AND e.archived = 0
        ...
        # Leaderboard rankings using Base 50 conversion
        cursor.execute(
            """
            ...
            FROM (
                ...
                FROM (
                    SELECT en.class_code, en.student_id,
                        AVG(COALESCE(50 + (ea.score / NULLIF((SELECT COUNT(*) FROM attempt_questions WHERE attempt_id = ea.attempt_id), 0) * 50), 0)) as student_avg
                    FROM enrollments en
                    LEFT JOIN exams e ON en.class_code = e.class_code
                    LEFT JOIN exam_attempts ea ON e.exam_id = ea.exam_id AND en.student_id = ea.student_id AND ea.status = 'finished'
        ...
        # ── Integration: Fetch voting status from Voxify ──────────────────
        ...
        try:
            vx_resp = _req.get(
                f"{VOXIFY_URL}/api/voters/status/{student_id}", timeout=3
            )
        ...
        try:
            ...
            ann_resp = _req.get(
                f"{VOXIFY_URL}/api/announcements", params=ann_params, timeout=3
            )
...
def profile():
    ...
    try:
        ...
        cursor.execute(
            "SELECT COUNT(*) as count FROM exam_attempts WHERE student_id = %s AND status = 'finished'",
            (user_id,),
        )
...
def student_exams():
    if user_logged_in():
        ...
        for exam in exams:
            ...
            end_time = (
                start_time + timedelta(minutes=exam["duration_minutes"])
                if start_time
                else None
            )
...
def save_progress():
    ...
    try:
        ...
        cursor.execute(
            "SELECT status FROM exam_attempts WHERE attempt_id = %s", (attempt_id,)
        )
        ...
        if not attempt or attempt["status"] == "blocked":
            return (
                jsonify(
                    {"status": "blocked", "message": "Access restricted by instructor."}
                ),
                403,
            )
        ...
        cursor.execute(
            "UPDATE exam_attempts SET current_q_index = %s WHERE attempt_id = %s",
            (current_idx, attempt_id),
        )
        connection.commit()
...
def take_exam(exam_id):
    ...
    try:
        ...
        cursor.execute(
            "SELECT * FROM exam_attempts WHERE student_id = %s AND exam_id = %s",
            (student_id, exam_id),
        )
        attempt = cursor.fetchone()
        ...
        for q in questions:
            cursor.execute(
                "SELECT * FROM options WHERE question_id = %s", (q["question_id"],)
            )
...
def review_results(attempt_id):
    ...
    # 1. SECURITY: Check if the teacher has enabled reviews for this specific exam
    cursor.execute(
        """
        SELECT e.allow_review
        FROM exams e
    ...
    if not exam_config or not exam_config.get("allow_review"):
        cursor.close()
        connection.close()
        flash(
            "The instructor has not enabled answer review for this assessment.",
            "warning",
        )
        return redirect(url_for("student.student_results"))

    # 2. Proceed to fetch questions served during this specific attempt
    cursor.execute(
        """
...
def review_exam(attempt_id):
    ...
    # 1. Fetch Attempt & Exam details (Including the allow_review permission flag)
    cursor.execute(
        """
        SELECT ea.*, e.title, e.pass_percentage, e.allow_review, c.course_name
        FROM exam_attempts ea
        JOIN exams e ON ea.exam_id = e.exam_id
        JOIN classes cl ON e.class_code = cl.class_code
        JOIN courses c ON cl.course_code = c.course_code
    ...
    # 2. SECURITY CHECK: Validate teacher permission
    if not attempt.get("allow_review"):
        cursor.close()
        connection.close()
        flash(
            "The instructor has not enabled answer review for this assessment.",
    ...
    # 3. Fetch the specific subset of questions served for this attempt
    cursor.execute(
        """
        SELECT q.*, sa.submitted_answer, sa.is_correct
        FROM questions q
    ...
    connection.close()
    return render_template(
        "student_review.html", attempt=attempt, questions=review_questions
    )
...
def log_violation():
    ...
    try:
        ...
        if violation_type in violations_to_count:
            cursor.execute(
                "UPDATE exam_attempts SET tab_switches = tab_switches + 1 WHERE attempt_id = %s",
                (attempt_id,),
            )
        ...
        cursor.execute(
            "SELECT tab_switches FROM exam_attempts WHERE attempt_id = %s",
            (attempt_id,),
        )
...
def submit_exam(attempt_id):
    ...
    try:
        ...
        cursor.execute(
            "SELECT status, exam_id FROM exam_attempts WHERE attempt_id = %s",
            (attempt_id,),
        )
        ...
        if not attempt_info or attempt_info["status"] == "blocked":
            session.pop("active_exam_id", None)
            flash(
                "Submission failed: Your access to this exam was restricted by the instructor.",
                "danger",
            )
            return redirect(url_for("student.student_exams"))
        ...
        for q in questions:
            ...
            cursor.execute(
                "SELECT submitted_answer FROM student_answers WHERE attempt_id = %s AND question_id = %s",
                (attempt_id, q_id),
            )
            ...
            if student_ans:
                ...
                if q_type in ("multiple_choice", "true_false"):
                    ...
                    for opt in options_list:
                        if opt["is_correct"]:
                            if (
                                student_ans_str == str(opt["option_id"])
                                or student_ans_str.lower()
                                == str(opt["option_text"]).strip().lower()
                            ):
                                is_correct = 1
                                break
...
def student_results():
    ...
    try:
        # Fetching scores with built-in Base 50 conversion
        cursor.execute("""
            SELECT
                ea.*,
                e.title,
...
def view_course(course_id):
    ...
    try:
        ...
        completed_exams = sum(
            1 for ex in course_exams if ex["attempt_status"] == "finished"
        )
        progress_pct = (
            int((completed_exams / total_exams) * 100) if total_exams > 0 else 0
        )

        now = datetime.now()
        for exam in course_exams:
            if exam["date_time"]:
                exam["end_time"] = exam["date_time"] + timedelta(
            ...
            exam["can_take"] = (
                exam["is_active"] == 1
                and not exam["is_upcoming"]
                and not exam["is_expired"]
                and exam["attempt_status"] not in ["finished", "blocked"]
            )
...
def student_analytics():
    ...
    try:
        # 1. COMPREHENSIVE STANDINGS (Leaderboard Logic updated to calculate averages on Base 50) [1]
        cursor.execute(
            """
            SELECT
                r.class_code,
            ...
            FROM (
                ...
                LEFT JOIN exam_attempts ea
                    ON en.student_id = ea.student_id AND ea.status = 'finished'
                LEFT JOIN exams e
                    ON ea.exam_id = e.exam_id
                WHERE
                    (e.archived = 0 OR e.archived IS NULL)
                GROUP BY en.class_code, en.student_id
            ) r
            JOIN students s
        ...
        for item in raw_items:
            ...
            success_rate = (
                round((item["correct_count"] / total) * 100, 1) if total > 0 else 0
            )
            ...
            item_analysis.append(
                {
                    ...
                    "incorrect_p": (
                        round((item["incorrect_count"] / total) * 100, 1)
                        if total > 0
                        else 0
                    ),
        ...
        # 3. EXAM STATISTICS (Pass/Fail Distribution updated with Base 50 conversion) [1]
        cursor.execute(
            """
            SELECT
                SUM(CASE WHEN (50 + (ea.score / NULLIF((SELECT COUNT(*) FROM attempt_questions WHERE attempt_id = ea.attempt_id), 0) * 50)) >= e.pass_percentage THEN 1 ELSE 0 END) as pass_count,
                SUM(CASE WHEN (50 + (ea.score / NULLIF((SELECT COUNT(*) FROM attempt_questions WHERE attempt_id = ea.attempt_id), 0) * 50)) < e.pass_percentage THEN 1 ELSE 0 END) as fail_count
            FROM exam_attempts ea
            JOIN exams e ON ea.exam_id = e.exam_id
            WHERE ea.student_id = %s AND ea.status = 'finished' AND e.archived = 0
        """,
...
def student_certificates():
    ...
    try:
        # Exam achievements (Base 50 converted)
        cursor.execute("""
            SELECT * FROM (
                SELECT
                    ...
                    RANK() OVER (PARTITION BY ea.exam_id ORDER BY (50 + (ea.score / NULLIF((SELECT COUNT(*) FROM attempt_questions WHERE attempt_id = ea.attempt_id), 0)) * 50) DESC) as exam_rank
                FROM exam_attempts ea
                JOIN exams e ON ea.exam_id = e.exam_id
                JOIN classes cl ON e.class_code = cl.class_code
                JOIN courses c ON cl.course_code = c.course_code
        ...
        # Course honors (Base 50 converted)
        cursor.execute("""
            SELECT * FROM (
                SELECT
                    en.student_id,
